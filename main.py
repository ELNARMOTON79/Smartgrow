...
class MainContent:
    ...
    def _create_views(self):
        ...
        # CUSTOM VIEW
        custom_frame = ctk.CTkFrame(self.content_container, fg_color=COLORS["card"], corner_radius=15)
        
        # Header
        ctk.CTkLabel(
            custom_frame, text="Vista Personalizada", font=ctk.CTkFont(size=22, weight="bold"),
            text_color=COLORS["text_dark"]
        ).pack(pady=20)
        
        # Content
        custom_content = ctk.CTkFrame(custom_frame, fg_color=COLORS["background"], corner_radius=10)
        custom_content.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        self.custom_label = ctk.CTkLabel(
            custom_content, text="", font=ctk.CTkFont(size=16),
            text_color=COLORS["text_light"]
        )
